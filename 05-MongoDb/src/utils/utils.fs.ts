...
import {} from "../Database/Schemas/user.model";
import {
  EmailOtp,
  Marks,
  Role,
...
export const matchOtpWithFile = async (email: string, otp: string) => {
  const otpDb = await OtpModel.findOne({ email });

  if (otpDb.otp != otp) return { status: 400, message: "Incorrect Otp." };
